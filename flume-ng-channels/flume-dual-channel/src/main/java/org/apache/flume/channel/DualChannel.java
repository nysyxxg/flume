 ...
 * https://www.aboutyun.com/thread-8318-1-1.html
 * (出处: about云开发)
 ...
 * 2.4 增加DualChannel
 * Flume本身提供了MemoryChannel和FileChannel。MemoryChannel处理速度快，但缓存大小有限，且没有持久化；
 * FileChannel则刚好相反。我们希望利用两者的优势，在Sink处理速度够快，Channel没有缓存过多日志的时候，
 * 就使用MemoryChannel，当Sink处理速度跟不上，又需要Channel能够缓存下应用端发送过来的日志时，
 * 就使用FileChannel，由此我们开发了DualChannel，能够智能的在两个Channel之间切换。
 */
...
public class DualChannel extends BasicChannelSemantics {
    ...
    public void configure(Context context) {
        ...
        /**
         * 我们在HdfsEventSink和DualChannel中增加开关，当开关打开的情况下， HdfsEventSink 不再往Hdfs上写数据，
         * 并且数据只写向DualChannel中的FileChannel。以此策略来防止Hdfs的正常停机维护。
         * switchon : 控制开关
         */
