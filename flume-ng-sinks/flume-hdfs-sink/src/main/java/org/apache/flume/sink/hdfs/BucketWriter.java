...
class BucketWriter {
    ...
    /**
     * Rename bucketPath file from .tmp to permanent location.
     * 基于美团使用对HDFSEventSink  进行了优化
     * 我们的HdfsSink增加了创建lzo文件后自动创建index功能。Hadoop提供了对lzo创建索引，
     * 使得压缩文件是可切分的，这样Hadoop Job可以并行处理数据文件。HdfsSink本身lzo压缩，
     * 但写完lzo文件并不会建索引，我们在close文件之后添加了建索引功能。
     */
