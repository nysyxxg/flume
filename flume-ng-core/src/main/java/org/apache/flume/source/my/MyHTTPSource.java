 ...
 * 自己修改 HTTPSource： 修改flume源码，使其HTTPSource具备访问路径功能
 * <p>
 * MyHTTPSource：这个就是主类了，里面有类似于main方法的start方法。
 * 其实本质上httpSource就是一个嵌入了jetty的服务器，通过接受post请求（目前写死了只处理post请求）。
 * 将数据转换为event往下发。所以，修改很简单。
 */
public class MyHTTPSource extends AbstractSource implements
    ...
    private static final Logger LOG = LoggerFactory.getLogger(MyHTTPSource.class);
    ...
    public void start() {
        ...
        try {
            ...
            context.addServlet(new ServletHolder(new FlumeHTTPServlet()), "/"+ contextPath);
